...
class PhaseScheduler(_LRScheduler):
    ...
    def __init__(
        ...
        lr_dict: Optional[Dict[int, float]] = None,  # {phase: lr}
        ...
        self.n_viewed_data = 0
        self.phase = 0
        self.alpha = 1.0

        super().__init__(optimizer, last_epoch)

    def step(self, n_viewed_data: Optional[int] = None) -> None:
        if n_viewed_data is not None:
            self.update(n_viewed_data)
        super().step()

    def update(self, n_viewed_data: int) -> None:
        """
        ...
        n_viewed_data: new phase         fade_epoch done        level_epoch done
        alpha:      increase from 0 ----------- 1 -------------------- 1
        resolution:        n  ----------------- n ------------------ 2 * n
        """
        self.n_viewed_data += n_viewed_data
        ...
        phase, phase_n_viewed_data = divmod(self.n_viewed_data, data_per_phase)

        if phase == 0:
            alpha = 1.0
        elif phase <= self.max_phase:
            alpha = min(phase_n_viewed_data / data_per_fade, 1.0)
        else:
            alpha = 1.0
    ...
    def get_lr(self) -> Tuple[float, ...]:  # type: ignore
        ...
        ramp_up = min(self.n_viewed_data / ramp_up_key_data, 1.0)
